...
anova_data <- function(timepoints, timepoints2, df){
  ...
  # Collapse data
  df3 <- summaryBy(y + event + n ~ timegroup + treatment + txCode + study + studyCode , FUN=c(sum, max), data=df2)
  df3 <- subset(df3, select=-c(event.max, n.max))
  names(df3) <- c("spgrp", "treatment", "trtn", "study", "studyn", "y", "nevents", "natrisk", "y.max")
